...
export function TextInputFG({ label, value, onChange, isRequired = false, placeholder = `Enter your ${label} here.` }) {
	return (
		<div class="form-group mb-2">
			<label htmlFor={`txtfg-${label}`} class="form-label">{label}</label>
			<input 
				type="text" 
...
export function TextAreaFG({ label, value, onChange, placeholder = `Enter your ${label} here. This uses markdown!` }) {
	return (
		<div class="form-group mb-2">
			<label htmlFor={`txtareafg-${label}`} class="form-label">{label}</label>
			<textarea 
				class="form-control" 
				rows="10" cols="80" 
				id={`txtareafg-${label}`}
...
export function TimeFG({ label, value, onChange }) {
	return (
		<div class="form-group mb-2">
			<label htmlFor={`timefg-${label}`} class="form-label">{label}</label>
			<input type="datetime-local" class="form-control" id={`timefg-${label}`} value={value} onChange={(e) => onChange(e.target.value)}></input>
		</div>
	);
}

export function CreateableMultiSelectFG({ label, isLoading, options, value, onChange, onCreate }) {
	return (
		<div class="form-group mb-2">
			<label htmlFor={`createmultiselfg-${label}`} class="form-label">{label}</label>
			<CreatableSelect
...
export function MultiSelectFG({ label, isLoading, options, value, onChange }) {
	return (
		<div class="form-group mb-2">
			<label htmlFor={`multiselfg-${label}`} class="form-label">{label}</label>
			<Select
				id={`multiselfg-${label}`}
				isClearable
				isMulti
...
export function SwitchFG({ label, checked, onChange }) {
	return (
		<div class="form-check form-switch">
			<input class="form-check-input" type="checkbox" id={`switchfg-${label}`} checked={checked} onChange={(e) => onChange(e.target.checked)}/>
			<label class="form-check-label" htmlFor={`switchfg-${label}`}>{label}</label>
		</div>
	)
}
