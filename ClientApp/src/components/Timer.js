...
const render2Start = ({ total, days, formatted, completed, props }) => {
	...
	if (completed) {
		if (props.comp.endTime) {
			...
			return <Countdown date={props.comp.endTime} renderer={render2End} duration={end - start}/>;
		}
		return <div class="text-center">Competition has started, but finish time is unknown!</div>;
	}
	...
	if (days < daysWarning) {
		return (
			...
			<span class="position-absolute inline-block start-50 translate-middle-x">
				Starts in: {formatted.days}:{formatted.hours}:{formatted.minutes}:{formatted.seconds}
			</span>
			<div class="progress" style={{height: "2em"}}>
				<div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" style={{width: `${total / milisecondsWarning * 100}%`}}></div>
			</div>
		</div>
		);
	}
	return <div class="text-center">Starts in: {formatted.days}:{formatted.hours}:{formatted.minutes}:{formatted.seconds}</div>;
}
...
const render2End = ({ total, formatted, completed, props }) => {
	...
	return (
		<div>
			<span class="position-absolute inline-block start-50 translate-middle-x">
				Finishes in: {formatted.days}:{formatted.hours}:{formatted.minutes}:{formatted.seconds}
			</span>
			<div class="progress" style={{height: "2em"}}>
				<div class="progress-bar progress-bar-striped progress-bar-animated bg-danger" role="progressbar" style={{width: `${total / props.duration * 100}%`}}></div>
			</div>
		</div>
	);
}