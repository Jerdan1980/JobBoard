...
export default function Tags() {
  ...
  return (
    <div>
      ...
      <div class="grid">
        {tags
          .filter((tag) => !(!showEmpty && tag.competitions.length === 0))
          .map((tag) => (
            <div
              class="g-col-12 g-col-sm-6 g-col-lg-4 g-col-xxl-3"
              key={tag.id}
            >
    ...
    <div class="card mb-3">
      <div class="card-body">
        ...
        <h6 class="card-subtitle mb-2 text-muted">
          # of comps: {tag.competitions.length}
        </h6>
        {tag.description && <Markdown contents={tag.description} />}
        <a href={`/competitions?tag=${tag.id}`} class="card-link">
          Explore tag
        </a>
        {loggedIn && (
          <a href={`/tags/edit?id=${tag.id}`} class="card-link">
            Edit tag
          </a>
        )}
      </div>
