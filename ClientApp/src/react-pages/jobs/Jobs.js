...
export default function Jobs() {
  ...
  const industryIdParam = queryParams
    .getAll('industry')
  ...
  const [selectedIndustries, setSelectedIndustries] = useQueryParams(
    industryIdParam,
    industries
  );
  ...
  return (
    <div class="">
      ...
      <div class="row">
        <div class="col">
          <h2>
            There are {jobs.filter((job) => filter(job)).length} Jobs that match
            your settings!
          </h2>
        </div>
      ...
      >
        ...
        <div class="offcanvas-body">
          ...
          <p>
            {jobs.filter((job) => filter(job)).length} Jobs that match your
            settings!
          </p>
  ...
  function JobCard({ job }) {
    ...
    const timerRenderer = ({ total, days, formatted, completed, props }) => {
      if (completed) return <div>Already Expired!</div>;
      return (
        <div>
          Expires in: {formatted.days}:{formatted.hours}:{formatted.minutes}:
          {formatted.seconds}
        </div>
      );
    };
    ...
    return (
      <div
        class={
          'card mb-3 ' + (job.id === id ? 'border-light' : 'border-primary')
        }
      >
        {job.fromApi && <div class="card-header">Automated</div>}
        <div class="card-body">
          ...
          <button
            ...
            class="btn btn-link stretched-link p-0"
          >
            Read More
          </button>
        </div>
