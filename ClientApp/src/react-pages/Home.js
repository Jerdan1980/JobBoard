...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch('/api/competitions/status')
      ...
      .then((data) => {
        ...
        data.forEach((comp) => {
          const isStarting = comp.startTime
  ...
  return (
    <>
      ...
      <p>
        Welcome to CNC! Our goal is to help you jumpstart your career in an
        industry of interest. Competitions are a good way of showing your
        interest <em>and</em> proving your skill.
      </p>
      <p>To help you get started, here are some quick links:</p>
      <ul>
        <li key="jobs">
          Looking for jobs? Here are our <a href="/jobs">careers</a>
        </li>
        <li key="comp">
          Want to get involved? Here are our{' '}
          <a href="/competitions">competitions</a>
        </li>
        <li key="tags">
          Have an interest in mind? Here are our <a href="/tags">tags</a>
        </li>
      </ul>
      ...
      <div className="row text-center">
        <a class="col" href="/tags">
          {numTags} Tags
        </a>
        <a class="col" href="/competitions">
          {totalComps} Competitions
        </a>
        <a class="col" href="/jobs">
          {totalJobs} Jobs
        </a>
        <a class="col" href="/industries">
          {industryCounts.length} Industries
        </a>
      </div>
      ...
      <div className="progress" style={{ height: '2em' }}>
        <div
          ...
          style={{ width: `${(startingComps / totalComps) * 100}%` }}
        >
          {startingComps} Starting
        </div>
        <div
          className="progress-bar bg-warning text-dark"
          style={{ width: `${(ongoingComps / totalComps) * 100}%` }}
        >
          {ongoingComps} Ongoing
        </div>
        <div
          className="progress-bar bg-success text-dark"
          style={{ width: `${(completedComps / totalComps) * 100}%` }}
        >
      ...
      {industryCounts.slice(0, 5).map((industry) => (
        <Progress
          className="mb-2"
          fraction={industry.count / totalJobs}
          name={industry.name}
          key={industry.name}
        />
      ))}
...
function Progress({ fraction, name, className }) {
  return (
    <div className={className}>
      <span className="position-absolute inline-block start-50 translate-middle-x">
